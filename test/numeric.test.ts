...
describe("examples from readme", () => {
  ...
  test("Metric", () => {
    ...
    expect(
      numberToString(1e-6, { metric: true, metricScale: metricScaleAscii })
    ).toBe("1u")
  ...
  test("Binary Metric", () => {
    ...
    expect(
      stringToNumber("64k", {
        metric: true,
        metricScale: metricScaleBinaryParse,
      })
    ).toBe(65536)
    expect(
    ...
    expect(
      numberToString(4096, { metric: true, metricScale: metricScaleBinary })
    ).toBe("4Ki")
    ...
    expect(
      numberToString(2 ** 21, {
        metric: true,
        metricScale: metricScaleBinaryShort,
      })
    ).toBe("2m")
  })
})
